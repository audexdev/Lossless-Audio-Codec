...
private:
    ...
    void mark_error();
    static uint32_t low_bits_mask(int bits);
};

inline uint32_t BitReader::low_bits_mask(int bits) {
    if (bits <= 0) return 0u;
...
inline BitReader::BitReader(const uint8_t* data, size_t size)
    : data(data), size(size), byte_pos(0), bit_pos(0), error(false) {}

inline BitReader::BitReader(const std::vector<uint8_t>& buf)
    : data(buf.data()), size(buf.size()), byte_pos(0), bit_pos(0), error(false) {}

inline void BitReader::mark_error() {
    this->error = true;
    this->byte_pos = this->size;
    this->bit_pos = 0;
}

inline uint32_t BitReader::read_bit() {
    if (this->byte_pos >= this->size) {
        this->mark_error();
        return 0;
    }

    const uint8_t byte = this->data[this->byte_pos];
    const int shift = 7 - this->bit_pos;
    const uint32_t bit = (byte >> shift) & 1u;
...
inline uint32_t BitReader::read_bits(int nbits) {
    if (nbits <= 0) return 0;

    if (this->error || this->byte_pos >= this->size) {
        this->mark_error();
        return 0;
    }
    ...
    if (this->bit_pos == 0) {
        ...
        if (nbits == 16 && this->size - this->byte_pos >= 2u) {
            const uint32_t value =
        ...
        if (nbits == 32 && this->size - this->byte_pos >= 4u) {
            const uint32_t value =
    ...
    if (static_cast<size_t>(nbits) > this->bits_remaining()) {
        this->mark_error();
        return 0;
    }
...
inline bool BitReader::read_unary_ones(uint32_t max_ones, uint32_t& ones) {
    ones = 0;
...
inline bool BitReader::consume_zero_padding_to_byte() {
...
inline bool BitReader::eof() const {
    return this->byte_pos >= this->size;
}

inline bool BitReader::has_error() const {
    return this->error;
}

inline size_t BitReader::bits_remaining() const {
